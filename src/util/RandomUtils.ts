...
export class RandomUtils {
     ...
     * @return {} 随机整数数组
     */
    public static nextIntArray(startInclusive: number, endExclusive: number, length: number): number[] {
     ...
     * @return {} 随机整数数组
     */
    public static nextFloatArray(startInclusive: number, endExclusive: number, length: number): number[] {
        if (endExclusive < startInclusive) {
            throw new IllegalArgumentError("起始值必须小于或等于结束值。");
        }
        if (startInclusive < 0) {
     ...
     * @return {} 随机字符串
     */
    public static nextString(startChar: string, endChar: string, length: number): string {
    ...
    private static randomArray(min: number, max: number, length: number): number[] {
